<?PHP
	...
	$ban = 0;
	$retorno="<legend>Lista de Empresas</legend>"; //$retorno=$retorno.$t;
	...
	//if($t=='E'){
			...
			$retorno=$retorno.'
				<div class="col-md-8">
					<div class="panel panel-primary">
					  ...
					  <div class="panel-body">
					    <p><strong>Usuario:</strong> '.$data2["usuario"].'</p>						    
					    <p><a href="../../vistas/empresa/modUsuarioE.php?u='.sha1($data3["idEmpresa"]).' &p=1" class="btn btn-primary"><span class="glyphicon glyphicon-pencil"></span> Modificar</a></p>
					  </div>
					</div>
				</div>
			';    
		...
		if($n>1){
			if($pag==1){
				...
				$l1='<li><a href="../../vistas/empresa/actualizarEmpresa.php?p='.($pag-1).'&n='.$_POST["nombre"].'">&laquo;</a></li>';
			}
			for($i=0;$i<$n;$i++){ 	
				if($pag==($i+1)){					  	
					$l2=$l2.'<li class="active"><a href="../../vistas/empresa/actualizarEmpresa.php?p='.($i+1).'&n='.$_POST["nombre"].'">'.($i+1).'<span class="sr-only">(current)</span></a></li>';
		  		} else{
		  			$l2=$l2.'<li><a href="../../vistas/empresa/actualizarEmpresa.php?p='.($i+1).'&n='.$_POST["nombre"].'">'.($i+1).'<span class="sr-only">(current)</span></a></li>';
				}
		  	}
		  	if($pag==$n){ 
				$l3=$l3.'<li class="disabled"><a href="#">&raquo;</a></li>';
			} else{
				$l3=$l3.'<li><a href="../../vistas/empresa/actualizarEmpresa.php?p='.($pag+1).'&n='.$_POST["nombre"].'">&raquo;</a></li>';
			}
			$retorno=$retorno.'
				<div class="col-md-8">
					<ul class="pagination">
