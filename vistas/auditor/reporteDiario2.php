<?php 
	...
	$sql="select * from Solicitud as a where 'APROVADO'=(select estado from Estado_solicitud where  a.idSolicitud=idSolicitud and fecha='".date("Y-m-d")."' order by estado desc limit 1) and '".$_POST["tipoC"]."'=(select tipo from Empresa where idEmpresa=a.idEmpresa)";
	$res=mysql_query($sql,$bd);	

	$sql2="select count(idSolicitud) as c from Solicitud as a where 'APROVADO'=(select estado from Estado_solicitud where  a.idSolicitud=idSolicitud and fecha='".date("Y-m-d")."' order by estado desc limit 1) and '".$_POST["tipoC"]."'=(select tipo from Empresa where idEmpresa=a.idEmpresa)";
	...
	<div class="row clearfix">
		<div class="col-md-12 column"> <!-- col-md-10 col-md-offset-2-->
			...
			<div class="row clearfix">
				...
				<form class="form-horizontal" id="reporteDiario2" name="reporteDiario2" action="reporteDiario2.php" method="post">
						<fieldset>
					...
					<div id="resultados">
						<legend>Lista de solicitudes aprobadas hoy (<?PHP echo date("d/m/Y"); ?>) empresas <?PHP if($_POST["tipoC"]=='o') echo "no exentas"; else echo "exentas"; ?></legend>
						<?PHP if($cantidad!=0){ ?>
						<h4>Total solicitudes aprobadas: <strong><?PHP echo $cantidad ?></strong> </h4>
						<?PHP
							while($data=mysql_fetch_assoc($res)){ 
								/*$sql="select estado, idUsuario from Estado_solicitud where idSolicitud=".$data["idSolicitud"]." order by fecha, estado desc limit 1";
								$res4=mysql_query($sql,$bd);
								$data4=mysql_fetch_assoc($res4);*/
								//if($data4["estado"]=="APROVADO"){
								...
								if($data2["tipo"]=='o'){
									...
									$dep='
									    ...
									    <p><strong>Fecha del dep&oacute;sito:</strong> '.$data6["fecha"].'</p>	
									    <p><strong>Numero de dep&oacute;sito:</strong> '.$data6["nDeposito"].'</p>
									    <p><strong>Dep&oacute;sito:</strong> <div id="links">
								...
								$sql="select nombre, apellido from Usuario where idUsuario=(select idUsuario from Estado_solicitud where idSolicitud=".$data["idSolicitud"]." and estado='APROVADO')";
							...
							<div class="col-md-8">
								<div class="panel panel-primary">
								  ...
								  <div class="panel-body">
								    ...
								    <?PHP echo $dep; ?>					    
								    <p><strong>Aprobada por:</strong> <?PHP echo $data5["nombre"]." ".$data5["apellido"]; ?> </p>
						...
						<?PHP } else{ ?>
							<div class="alert alert-danger alert-dismissable">
								...
								<strong> No se aprobaron solicitudes hoy.</strong>
							</div>
						<?PHP } ?>
