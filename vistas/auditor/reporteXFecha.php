...
<head>
  <meta charset="utf-8">
  <title>Reporte solicitudes por fecha</title>
			...
			<div class="row clearfix">
				...
				<div class="col-md-9 column">
					<form class="form-horizontal" id="consultaSolicitudF" name="consultaSolicitudF" action="#" method="post">
						...
						<!-- Form Name -->
						<legend>Consulta solicitudes por fecha</legend>
	...
	$(document).ready(function(){
      ...
      $('#fechaF').datepicker()
		...
		$('#buscar').click(function(){ 
			...
			if(($('#f1').val()!="" || $('#f2').val()!="") && !ban){ 
				var emp = $('#empresa').val();    
				var t = $("input:radio[name ='tipoC']:checked").val();    
				var dataString = 'tipo='+t;	 
				var p;
				if($('#f1').val()=="" && $('#f2').val()!=""){
					p = '1';
				}
				...
				$.ajax({ 
					type: "POST",
					url: "../../modeloControlador/auditor/consultSolXFec.php?P="+p+"&f1="+$('#f1').val()+"&f2="+$('#f2').val(),
					data: dataString,
					success: function(data) {	
						if(data=="-1"){ 
							$("#alerta").html('<div id="errorBU" class="alert alert-danger alert-dismissable">'+
							'<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+
							'<strong> No se encontraron solicitudes en el rango de fechas seleccionado </strong>'+
							'</div>');
						}
						else{
